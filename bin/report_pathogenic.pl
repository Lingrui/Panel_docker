...
if (-s $ARGV[0]){
	...
	$out ='{"disease":"","result":[';
		while(<INPUT>){
			...
			$symble = $candidates[2];
			$aa_mut = $candidates[13];
			$nt_mut = $candidates[14];
			$rs_id  = $candidates[16];
			$position = $candidates[26];
			$pathogenicity = $candidates[-1];
			my $pmid  = $candidates[20];
			foreach(@anno){
				undef @w;
				@w=split(/\t/,$_);
			...
			my @pmid_list = split (/,/,$pmid);
			foreach(@pmid_list){
				$pl=$_;
				if ($pmd{$pl}){
				my @description_output =split (/\t/,$pmd{$pl});
					...
					$reference_output .= "$auther等在$magzine中发表论文表明，$symble基因的$nt_mut突变导致$mut_type$aa_mut，可能与$disease相关，请参阅：<br/>$description_output[-1]<br/>";
					 ...
					 $reference_output .= "请参阅PMID: $pl<br/>"}
				else {$reference_output .="请参阅: $pl<br/>"}
			}
			...
			$out .='{'.
				...
				'"heterozygous":"' . $het . '",'.
				'"type":"'. $mut_type . '",'.
				'"pathogenicity":"'. "$disease " ."****$pathogenicity****". '",'.
